...
```
3. 서버 정보 추철
- 웹서버에는 웹 애플리케이션당 하나씩 ServletContext 객체가 생성되어 있고, 이를 통해 웹 애플리케이션에 대한 정보를 추출할 수 있다. 
